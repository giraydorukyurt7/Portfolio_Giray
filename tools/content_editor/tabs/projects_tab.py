...
class ProjectsTab(ListEntityTab):
    ...
    def build_form(self, parent):
        ...
        f.columnconfigure(0, weight=1); f.rowconfigure(1, weight=1); f.rowconfigure(3, weight=1); f.rowconfigure(6, weight=1); f.rowconfigure(7, weight=1)
        return f
    ...
    def set_form(self, rec: Dict[str, Any]):
        self.title_ml.set(rec.get("title")); self.summary_ml.set(rec.get("summary"))
        self.dr.set(rec.get("start"), rec.get("end"), rec.get("present", False))
        self.stack.set(", ".join(rec.get("stack", [])))
        links = rec.get("links", {})
        self.link_gh.set(links.get("github")); self.link_demo.set(links.get("demo"))
        self.highlights_ml.set(rec.get("highlights"))
        self.images.set("".join(rec.get("images", [])))
